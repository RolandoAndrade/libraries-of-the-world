...
public class LibraryService {
    ...
    public LibraryService(LibraryRepository repository, LoggerService loggerService, LibraryCommandSet commandSet) {
    ...
    public Book getBook(String name) throws BookNotFoundException {
        ...
        Book book = this.repository.request(this.commandSet.getBookCommand(), name);
        if (book == null) {
    ...
    public List<Book> getAuthor(String name, String surname) throws ThereAreNoBooksException {
        ...
        List<Book> books = this.repository.request(name, surname);
        if (books.size() == 0) {
